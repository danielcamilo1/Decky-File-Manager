...
* **Copy progress that tells you something** — the modal shows the speed as it happens, on a live graph, with how many files are left, how much is left to move and how long it should take. **Cancel** stops the copy where it is: files already copied stay, the half-written one is cleaned up, and a cancelled move has not touched the originals. When it finishes it says so — the modal turns green on **Complete**, and a note above the list gives the size copied and how long it took.
* **Mount a drive from Gaming Mode** — a USB stick plugged in while the plugin is open shows up on its own as a dashed chip; pressing it mounts the drive and opens it.
* **Safe eject** — **Eject drive** in the **Y** menu unmounts every partition on the drive and powers it down, then tells you it is safe to unplug. Any drive that is mounted can be ejected, not only the one you are browsing.
