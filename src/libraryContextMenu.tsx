/**
 * Adds a "Browse local files" submenu to the context menu Steam shows for a
 * game — the same menu SteamGridDB puts "Change Artwork..." in, reached with
 * the Options button on a game in the library.
 *
...
import { openGameFolder } from "./gameFolders";

/** Marks our injected nodes so repeated renders do not stack duplicates. */
const MARKER = "decky-file-manager-browse";
const LOG = "[decky-file-manager]";

/** What the finder settled on, for the log line on load. */
export let menuPatchState: "patched" | "not-found" | "failed" | "idle" = "idle";

function folderItems(appid: number, prefix: boolean) {
  const label = (name: string) => (prefix ? `${t("menu.browse_local_files")}: ${name}` : name);
  return [
    <MenuItem
      key={`${MARKER}-install`}
      onSelected={() => openGameFolder({ appid: String(appid), kind: "install" })}
    >
      {label(t("menu.installation_folder"))}
    </MenuItem>,
    <MenuItem
      key={`${MARKER}-compat`}
      onSelected={() => openGameFolder({ appid: String(appid), kind: "compat" })}
    >
      {label(t("menu.compatdata_folder"))}
    </MenuItem>,
  ];
}

/**
 * The nodes to splice in: one submenu, or the two folders spelled out flat if
 * Steam's submenu component could not be found.
 */
function browseNodes(appid: number) {
  if (SubMenu) {
    return [
      <SubMenu key={MARKER} label={t("menu.browse_local_files")}>
        {folderItems(appid, false)}
      </SubMenu>,
    ];
  }
  return folderItems(appid, true);
}

/** Insert ahead of "Properties...", where the other plugins' entries go. */
function spliceMenuItems(items: any[], appid: number) {
  const propertiesIdx = items.findIndex((item: any) =>
    findInReactTree(
      item,
      (node: any) => node?.onSelected && String(node.onSelected).includes("AppProperties"),
    ),
  );
  const nodes = browseNodes(appid);
  if (propertiesIdx >= 0) items.splice(propertiesIdx, 0, ...nodes);
  else items.push(...nodes);
}
...
function removeExisting(items: any[]) {
  for (let index = items.length - 1; index >= 0; index -= 1) {
    if (typeof items[index]?.key === "string" && items[index].key.startsWith(MARKER)) {
...
function patchMenuItems(items: any[], fallbackAppid: number | undefined) {
  ...
  spliceMenuItems(items, appid as number);
}
...
export function patchLibraryContextMenu(): () => void {
  ...
  try {
    outer = afterPatch(LibraryContextMenu.prototype, "render", (_args: any[], component: any) => {
