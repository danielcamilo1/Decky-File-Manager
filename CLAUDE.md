...
`t(key)` reads from a translation table **hardcoded inside [src/i18n.ts](src/i18n.ts)** (en, pt-BR, es, zh-CN, fr, de; 184 keys each, and the six blocks must agree). The [locales/](locales/) JSON files are a separate runtime-overlay path: `loadRemoteTranslations` fetches `./locales/<locale>.json` (then any `translations_base_url` from plugin.json, then a raw GitHub URL) and caches the result in `localStorage` for 24h.
